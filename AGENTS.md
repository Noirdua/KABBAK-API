...
Public without a key: `/api/v1/health*`, `/branding`, non-tarot `/assets/img`, and the inbound webhooks `/api/v1/webhooks/email/{resend,generic}` and `/api/v1/webhooks/stripe` (signature/token verified, own rate limit). Trial account routes are core and pre-auth too: `GET /auth/providers`, `POST /auth/challenge|signup|verify|resend|login|forgot|reset`, and the email link `GET /auth/verify`. DLC plugins can add their own public routes via a manifest `server` entry (mounted at `/api/v1/plugins/<name>/server/…`) — e.g. the `demo-users` plugin serves `/demo-access`.
