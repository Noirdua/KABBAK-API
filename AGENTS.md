...
Public without a key: `/api/v1/health*`, `/branding`, non-tarot `/assets/img`. Trial account routes are core and pre-auth too: `GET /auth/providers`, `POST /auth/challenge|signup|verify|resend|login|forgot|reset`, and the email link `GET /auth/verify`. DLC plugins can add their own public routes via a manifest `server` entry (mounted at `/api/v1/plugins/<name>/server/…`) — e.g. the `demo-users` plugin serves `/demo-access`.
